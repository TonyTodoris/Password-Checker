...
def get_password_leaks_count(hashes, hask_to_check):
    '''
    Function that checks the results of the hashed password,
    and returns the count the passwords have been breached/hacked
    '''
...
def check_pwned_api(password):
    '''
    Function that hashes in SHA1 algorithm the password,
    and then returns the count that the password might have been breached
    '''
    sha1password = hashlib.sha1(password.encode('utf8')).hexdigest().upper()
    # The API is using the k-anonymity technique
    first_five_chars, remaining_chars = sha1password[:5], sha1password[5:]
